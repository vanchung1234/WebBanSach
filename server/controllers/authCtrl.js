...
const authCtrl = {
    register: async (req, res) => {
        try {
            ...
            if(user) return res.status(400).json({msg: "This user name already exists."})
    ...
    login: async (req, res) => {
        try {
            ...
            if(!user) return res.status(400).json({msg: "This username or password wrong."})

            const isMatch = await bcrypt.compare(password, user.password)
            if(!isMatch) return res.status(400).json({msg: "This username or password wrong."})
